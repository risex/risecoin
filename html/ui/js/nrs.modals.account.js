...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.processAccountModalData = function(account) {
		if (account.unconfirmedBalanceNQT == "0") {
			...
			$("#user_info_modal_account_balance").html(NRS.formatAmount(account.unconfirmedBalanceNQT) + " RISE");
		}
		...
		NRS.sendRequest("getAccountTransactions", {
			...
			if (response.transactions && response.transactions.length) {
				...
				for (var i = 0; i < response.transactions.length; i++) {
					...
					if (/^RISE\-/i.test(NRS.userInfoModal.user)) {
		...
		NRS.sendRequest("getDGSGoods", {
			...
			if (response.goods && response.goods.length) {
				for (var i = 0; i < response.goods.length; i++) {
					...
					rows += "<tr><td><a href='#' data-goto-goods='" + String(good.goods).escapeHTML() + "' data-seller='" + String(NRS.userInfoModal.user).escapeHTML() + "'>" + String(good.name).escapeHTML() + "</a></td><td>" + NRS.formatAmount(good.priceNQT) + " RISE</td><td>" + NRS.format(good.quantity) + "</td></tr>";
				}
			}
