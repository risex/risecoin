...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.forms.sellAlias = function($modal) {
		...
		if (data.modal == "cancel_alias_sale") {
			...
			if (data.recipient == NRS.genesisRS) {
				if (!data.priceRISE || data.priceRISE == "0") {
					return {
						"error": $.t("error_not_specified", {
		...
		NRS.sendRequest("getAlias", {
			...
			if (response.errorCode) {
				...
				if (!("priceNQT" in response)) {
					...
					$modal.find("input[name=amountRISE]").val(NRS.convertToRISE(response.priceNQT)).prop("readonly", true);
				}
			}
		}, false);
	});
	...
	NRS.forms.buyAliasError = function() {
		$("#buy_alias_modal").find("input[name=priceRISE]").prop("readonly", false);
	}
		...
		if (alias) {
			...
			NRS.sendRequest("getAlias", {
				...
				if (response.errorCode) {
					...
					} else if ((aliasURI = /acct:(.*)@rise/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
	...
	NRS.forms.setAlias = function($modal) {
		...
		if (data.type == "account") {
			if (!(/acct:(.*)@rise/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
				if (/^(RISE\-)/i.test(data.aliasURI)) {
					...
					if (!address.set(data.aliasURI)) {
						...
						data.aliasURI = "acct:" + data.aliasURI + "@rise";
					}
	...
	function setAliasType(type, uri) {
		...
		if (type == "uri") {
			...
			$("#register_alias_uri").val("").mask("RISE-****-****-****-*****");

			if (uri) {
				var match = uri.match(/acct:(.*)@rise/i);
				if (!match) {
				...
				} else if (!/^RISE\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
					uri = NRS.accountRS;
				}
					...
					if ("priceNQT" in response) {
						if (response.buyer == NRS.account) {
							message = $.t("alias_sale_direct_offer", {
								"rise": NRS.formatAmount(response.priceNQT)
