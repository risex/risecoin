...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.checkAliasVersions = function() {
		...
		if (NRS.inApp) {
			...
			if (NRS.appPlatform == "" || NRS.appVersion == "" || version_compare(NRS.appVersion, "2.0.0", "<")) {
				$("#secondary_dashboard_message").removeClass("alert-success").addClass("alert-danger").html("A new version of the RISE Wallet application is available for download <a href='http://rise.org/get-started-rise/download-rise-software' target='_blank'>here</a>. You must install it manually due to changes in the NRS startup procedure.").show();
			}
	...
	NRS.downloadClientUpdate = function(version) {
		...
		if (NRS.inApp) {
			...
			$("#nrs_update_iframe").attr("src", "https://bitbucket.org/JeanLucPicard/rise/downloads/rise-client-" + NRS.downloadedVersion.versionNr + ".zip");
