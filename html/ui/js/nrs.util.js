...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.calculateOrderPricePerWholeQNT = function(price, decimals, returnAsObject) {
		...
		return NRS.convertToRISE(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	}
	...
	NRS.calculateOrderTotal = function(quantityQNT, priceNQT) {
		...
		return NRS.convertToRISE(quantityQNT.multiply(priceNQT));
	}
	...
	NRS.convertToRISE = function(amount, returnAsObject) {
		var negative = "";
		var afterComma = "";
	...
	NRS.formatAmount = function(amount, round, no_escaping) {
		...
		if (typeof amount == "object") {
			var params = NRS.convertToRISE(amount, true);
		...
		*/
		...
		for (var key in data) {
			...
			if (/_formatted_html$/i.test(key)) {
				...
				value = NRS.formatAmount(new BigInteger(String(value))) + " RISE";
