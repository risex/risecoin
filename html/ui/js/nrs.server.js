...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.sendRequest = function(requestType, data, callback, async) {
		...
		//convert RISE to NQT...
		try {
			var riseFields = ["feeRISE", "amountRISE", "priceRISE", "refundRISE", "discountRISE"];

			for (var i = 0; i < riseFields.length; i++) {
				var riseField = riseFields[i];
				var field = riseField.replace("RISE", "");

				if (riseField in data) {
					data[field + "NQT"] = NRS.convertToNQT(data[riseField]);
					delete data[riseField];
				}
			}
		} catch (err) {
			if (callback) {
		...
		if (requestType == "getAccountId") {
			...
			var riseAddress = new RiseAddress();

			if (riseAddress.set(accountId)) {
				var accountRS = riseAddress.toString();
			} else {
				var accountRS = "";
			}
	...
	NRS.processAjaxRequest = function(requestType, data, callback, async) {
		...
		var url = NRS.server + "/rise?requestType=" + requestType;
	...
	NRS.verifyAndSignTransactionBytes = function(transactionBytes, signature, requestType, data) {
		...
		if (!("recipient" in data)) {
			...
			data.recipientRS = "RISE-MRCC-2YLS-8M54-3CMAJ";
		}
	...
	NRS.broadcastTransactionBytes = function(transactionData, callback, originalResponse, originalData) {
		$.ajax({
			url: NRS.server + "/rise?requestType=broadcastTransaction",
			crossDomain: true,
			dataType: "json",
			type: "POST",
			timeout: 30000,
