...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.forms.addAssetBookmark = function($modal) {
		...
		if (!/^\d+$/.test(data.id) && !/^RISE\-/i.test(data.id)) {
			return {
				"error": $.t("error_asset_or_account_id_invalid")
			};
		}

		if (/^RISE\-/i.test(data.id)) {
	...
	NRS.loadAsset = function(asset, refresh) {
		...
		if (!refresh) {
			...
			$("#sell_asset_for_rise").html($.t("sell_asset_for_rise", {
				"assetName": String(asset.name).escapeHTML()
			}));
			$("#buy_asset_with_rise").html($.t("buy_asset_with_rise", {
				"assetName": String(asset.name).escapeHTML()
			}));
		...
		if (NRS.accountInfo.unconfirmedBalanceNQT == "0") {
			$("#your_rise_balance").html("0");
			$("#buy_automatic_price").addClass("zero").removeClass("nonzero");
		} else {
			$("#your_rise_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceNQT));
	...
	$("#asset_exchange_bid_orders_table tbody, #asset_exchange_ask_orders_table tbody").on("click", "td", function(e) {
		...
		try {
			...
			$("#" + type + "_asset_total").val(NRS.convertToRISE(totalNQT));
		} catch (err) {
			return;
		}
	...
	$("#sell_automatic_price, #buy_automatic_price").on("click", function(e) {
		try {
			...
			if (price.cmp(new Big("0")) <= 0) {
				...
				$("#" + type + "_asset_price").val(NRS.convertToRISE(price.toString()));
			}
			...
			$("#" + type + "_asset_total").val(NRS.convertToRISE(total.toString()));
	...
	$("#asset_order_modal").on("show.bs.modal", function(e) {
		...
		try {
			...
			var totalRISE = NRS.formatAmount(NRS.calculateOrderTotalNQT(quantityQNT, priceNQT, NRS.currentAsset.decimals), false, true);
		} catch (err) {
		...
		if (orderType == "buy") {
			var description = $.t("buy_order_description", {
				...
				"rise": NRS.formatAmount(priceNQTPerWholeQNT)
			});
			var tooltipTitle = $.t("buy_order_description_help", {
				"rise": NRS.formatAmount(priceNQTPerWholeQNT, false, true),
				"total_rise": totalRISE
			});
		} else {
		...
		$("#asset_order_total").html(totalRISE + " RISE");
		$("#asset_order_fee_paid").html(NRS.formatAmount(feeNQT) + " RISE");
