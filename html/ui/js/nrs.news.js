		...
		if (currentTime - NRS.newsRefresh > 60 * 60 * 10) { //10 minutes before refreshing..
			...
			var settingsReddit = {
				...
				"filter": function(entry, tokens) {
					return tokens.title.indexOf("Donations toward") == -1 && tokens.title.indexOf("RISE tipping bot has arrived") == -1
				}
			};

			$("#riseforum_news").rss("https://riseforum.org/index.php?type=rss;action=.xml;sa=news;", settings, NRS.newsLoaded);
			$("#reddit_news").rss("http://www.reddit.com/r/RISE/.rss", settingsReddit, NRS.newsLoaded);
			$("#risecoin_blogspot_news").rss("http://risecoin.blogspot.com/feeds/posts/default", settings, NRS.newsLoaded);
			$("#riseer_news").rss("http://riseer.org/feed/", settings, NRS.newsLoaded);
			$("#risecommunity_news").rss("http://www.risecommunity.org/rss.xml", settings, NRS.newsLoaded);
		}
