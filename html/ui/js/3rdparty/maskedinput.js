...
! function($) {
			...
			return !mask && this.length > 0 ? (input = $(this[0]), input.data($.mask.dataName)()) : (settings = $.extend({
				...
				firstNonMaskPos = null, $.each(mask.split(""), function(i, c) {
					...
					function keydownEvent(e) {
						...
						if (e.keyCode == 8) {
							...
							if (settings.unmask !== false) {
								//backspace, remove
								if ((pos.begin == 0 && pos.end == 24) || (currentInput == "RISE-____-____-____-_____" && pos.begin == 4)) {
									input.val("");
									$(this).trigger("unmask");
									return;
								}
					...
					function keypressEvent(e) {
						...
						if (/^RISE\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(input.val())) {
					...
					function checkVal(allow) {
						input.val(input.val().replace(/^\s*RISE\-\s*RISE/i, "RISE-"));
					...
					if (settings.noMask) {
						input.bind("keyup.remask", function(e) {
							if (input.val().toLowerCase() == "rise-") {
								input.val("").mask("RISE-****-****-****-*****").unbind(".remask").trigger("focus");
							}
						}).bind("paste.remask", function(e) {
							setTimeout(function() {
								var newInput = input.val();

								if (/^RISE\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(newInput) || /^RISE[A-Z0-9]{17}/i.test(newInput)) {
									input.mask("RISE-****-****-****-*****").trigger("checkRecipient").unbind(".remask");
								}
							}, 0);
						});
						return;
					...
					input.data($.mask.dataName, function() {
						...
						setTimeout(function() {
							...
							if (/^RISE\-[0-9]{19,20}$/i.test(pasted)) {
								//old style accounts..
								input.val("").trigger("oldRecipientPaste");
							} else {
								var match = /^RISE\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.exec(pasted);

								if (match && match[0]) {
									input.val(match[0]).trigger("checkRecipient");
								} else {
									match = /^RISE[A-Z0-9]{17}/i.exec(pasted);
									if (match && match[0]) {
	...
	function text_diff(first, second) {
		...
		if (/^RISE\-/i.test(second) && !/^RISE\-/i.test(diff)) {
			diff = "RISE-" + diff;
		}
