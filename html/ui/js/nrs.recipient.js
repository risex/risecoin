...
var NRS = (function(NRS, $, undefined) {
	NRS.automaticallyCheckRecipient = function() {
		...
		$recipientFields.on("checkRecipient", function() {
			...
			if (value && value != "RISE-____-____-____-_____") {
	...
	$("#send_money_amount").on("input", function(e) {
		...
		$(this).closest(".modal").find(".advanced_fee").html(NRS.formatAmount(NRS.convertToNQT(fee)) + " RISE");
	});
	*/
			...
			if (response.publicKey) {
				callback({
					...
					"message": $.t("recipient_info", {
						"rise": NRS.formatAmount(response.unconfirmedBalanceNQT, false, true)
					}),
					"account": response
				});
			} else {
				if (response.errorCode) {
					if (response.errorCode == 4) {
						callback({
							"type": "danger",
							"message": $.t("recipient_malformed") + (!/^(RISE\-)/i.test(accountId) ? " " + $.t("recipient_alias_suggestion") : ""),
							"account": null
						});
	...
	NRS.checkRecipient = function(account, modal) {
		...
		if (/^(RISE\-)?[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(account)) {
