...
import static rise.http.JSONResponses.*;
    ...
    static int getNumberOfConfirmations(HttpServletRequest req) throws ParameterException {
        ...
        if (numberOfConfirmationsValue != null) {
            try {
                ...
                if (numberOfConfirmations <= Rise.getBlockchain().getHeight()) {
    ...
    static int getHeight(HttpServletRequest req) throws ParameterException {
        ...
        if (heightValue != null) {
            try {
                ...
                if (height < Rise.getBlockchainProcessor().getMinRollbackHeight()) {
    ...
    static Transaction parseTransaction(String transactionBytes, String transactionJSON) throws ParameterException {
        ...
        if (transactionBytes != null) {
            try {
                ...
                return Rise.getTransactionProcessor().parseTransaction(bytes);
            ...
            try {
                ...
                return Rise.getTransactionProcessor().parseTransaction(json);
            } catch (RiseException.ValidationException | RuntimeException | ParseException e) {
                Logger.logDebugMessage(e.getMessage(), e);
                JSONObject response = new JSONObject();
                response.put("errorCode", 4);
