...
import static rise.http.JSONResponses.INCORRECT_BLOCK;
import static rise.http.JSONResponses.INCORRECT_HEIGHT;
import static rise.http.JSONResponses.INCORRECT_TIMESTAMP;
import static rise.http.JSONResponses.UNKNOWN_BLOCK;
...
public final class GetBlock extends APIServlet.APIRequestHandler {
    ...
    JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        if (blockValue != null) {
            try {
                blockData = Rise.getBlockchain().getBlock(Convert.parseUnsignedLong(blockValue));
            ...
            try {
                ...
                if (height < 0 || height > Rise.getBlockchain().getHeight()) {
                    return INCORRECT_HEIGHT;
                }
                blockData = Rise.getBlockchain().getBlockAtHeight(height);
            } catch (RuntimeException e) {
                return INCORRECT_HEIGHT;
            }
            ...
            try {
                ...
                blockData = Rise.getBlockchain().getLastBlock(timestamp);
            } catch (RuntimeException e) {
                return INCORRECT_TIMESTAMP;
            }
        } else {
            blockData = Rise.getBlockchain().getLastBlock();
        }
