...
import rise.util.CountingInputStream;
import rise.util.CountingOutputStream;
...
final class PeerImpl implements Peer {
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < Rise.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceNQT();
            hallmarkBalanceHeight = Rise.getBlockchain().getHeight();
        }
        return (int)(adjustedWeight * (hallmarkBalance / Constants.ONE_RISE) / Constants.MAX_BALANCE_RISE);
    }
    ...
    public void blacklist(Exception cause) {
        if (cause instanceof RiseException.NotCurrentlyValidException || cause instanceof BlockchainProcessor.BlockOutOfOrderException
    ...
    boolean analyzeHallmark(String address, final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.MAX_BALANCE_RISE * peer.getHallmarkWeight(mostRecentDate) / totalWeight;
