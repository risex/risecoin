...
public interface Appendix {
    ...
    static abstract class AbstractAppendix implements Appendix {
        ...
        abstract void validate(Transaction transaction) throws RiseException.ValidationException;
    ...
    public static class Message extends AbstractAppendix {
        ...
        static Message parse(JSONObject attachmentData) throws RiseException.NotValidException {
        ...
        Message(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
            ...
            if (messageLength > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new RiseException.NotValidException("Invalid arbitrary message length: " + messageLength);
            }
        ...
        @Override
        void validate(Transaction transaction) throws RiseException.ValidationException {
            if (this.isText && transaction.getVersion() == 0) {
                throw new RiseException.NotValidException("Text messages not yet enabled");
            }
            if (transaction.getVersion() == 0 && transaction.getAttachment() != Attachment.ARBITRARY_MESSAGE) {
                throw new RiseException.NotValidException("Message attachments not enabled for version 0 transactions");
            }
            if (message.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new RiseException.NotValidException("Invalid arbitrary message length: " + message.length);
            }
        }
    ...
    abstract static class AbstractEncryptedMessage extends AbstractAppendix {
        ...
        private AbstractEncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
            super(buffer, transactionVersion);
        ...
        void validate(Transaction transaction) throws RiseException.ValidationException {
            if (encryptedData.getData().length > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
                throw new RiseException.NotValidException("Max encrypted message length exceeded");
            }
            if ((encryptedData.getNonce().length != 32 && encryptedData.getData().length > 0)
                ...
                throw new RiseException.NotValidException("Invalid nonce length " + encryptedData.getNonce().length);
            }
        }
    ...
    public static class EncryptedMessage extends AbstractEncryptedMessage {
        ...
        static EncryptedMessage parse(JSONObject attachmentData) throws RiseException.NotValidException {
        ...
        EncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws RiseException.ValidationException {
            super(buffer, transactionVersion);
        }

        EncryptedMessage(JSONObject attachmentData) throws RiseException.NotValidException {
        ...
        void validate(Transaction transaction) throws RiseException.ValidationException {
            ...
            if (transaction.getRecipientId() == 0) {
                throw new RiseException.NotValidException("Encrypted messages cannot be attached to transactions with no recipient");
            }
            if (transaction.getVersion() == 0) {
                throw new RiseException.NotValidException("Encrypted message attachments not enabled for version 0 transactions");
            }
        }
    ...
    public static class EncryptToSelfMessage extends AbstractEncryptedMessage {
        ...
        static EncryptToSelfMessage parse(JSONObject attachmentData) throws RiseException.NotValidException {
        ...
        EncryptToSelfMessage(ByteBuffer buffer, byte transactionVersion) throws RiseException.ValidationException {
            super(buffer, transactionVersion);
        }

        EncryptToSelfMessage(JSONObject attachmentData) throws RiseException.NotValidException {
        ...
        void validate(Transaction transaction) throws RiseException.ValidationException {
            super.validate(transaction);
            if (transaction.getVersion() == 0) {
                throw new RiseException.NotValidException("Encrypt-to-self message attachments not enabled for version 0 transactions");
            }
        }
    ...
    public static class PublicKeyAnnouncement extends AbstractAppendix {
        ...
        static PublicKeyAnnouncement parse(JSONObject attachmentData) throws RiseException.NotValidException {
        ...
        void validate(Transaction transaction) throws RiseException.ValidationException {
            if (transaction.getRecipientId() == 0) {
                throw new RiseException.NotValidException("PublicKeyAnnouncement cannot be attached to transactions with no recipient");
            }
            if (publicKey.length != 32) {
                throw new RiseException.NotValidException("Invalid recipient public key length: " + Convert.toHexString(publicKey));
            }
            long recipientId = transaction.getRecipientId();
            if (Account.getId(this.publicKey) != recipientId) {
                throw new RiseException.NotValidException("Announced public key does not match recipient accountId");
            }
            if (transaction.getVersion() == 0) {
                throw new RiseException.NotValidException("Public key announcements not enabled for version 0 transactions");
            }
            ...
            if (recipientAccount != null && recipientAccount.getPublicKey() != null && ! Arrays.equals(publicKey, recipientAccount.getPublicKey())) {
                throw new RiseException.NotCurrentlyValidException("A different public key for this account has already been announced");
            }
        }
