...
final class BlockchainImpl implements Blockchain {
    ...
    private final AtomicReference<BlockImpl> lastPOSBlock = new AtomicReference<>();
    private final AtomicReference<BlockImpl> lastHDDBlock = new AtomicReference<>();
    ...
    void setLastBlock(BlockImpl block) {
        ...
        if(block.getHeight() == 0) {
        	lastPOSBlock.set(block);
        	lastHDDBlock.set(block);
        }
        else {
        	if(block.getNonce() == -1) {
        		lastPOSBlock.set(block);
    ...
    void setLastBlocks() {
    	BlockImpl POSBlock = BlockDb.findLastBlockForGen(true);
    	BlockImpl HDDBlock = BlockDb.findLastBlockForGen(false);
    	if(POSBlock != null) {
    		lastPOSBlock.set(POSBlock);
    	}
    	else {
    		lastPOSBlock.set(BlockDb.findLastBlock());
    	}
    	lastHDDBlock.set(HDDBlock);
    }
    ...
    public BlockImpl getLastPOSBlock() {
    	return lastPOSBlock.get();
    }
    
    @Override
    public BlockImpl getLastHDDBlock() {
    	return lastHDDBlock.get();
    }
    ...
    public DbIterator<BlockImpl> getBlocks(Connection con, PreparedStatement pstmt) {
        return new DbIterator<>(con, pstmt, new DbIterator.ResultSetReader<BlockImpl>() {
            @Override
            public BlockImpl get(Connection con, ResultSet rs) throws RiseException.ValidationException {
    ...
    public List<BlockImpl> getBlocksAfter(long blockId, int limit) {
        ...
        try (Connection con = Db.db.getConnection();
            ...
            return result;
        } catch (RiseException.ValidationException|SQLException e) {
            throw new RuntimeException(e.toString(), e);
        }
    }
    ...
    public DbIterator<TransactionImpl> getTransactions(Account account, int numberOfConfirmations, byte type, byte subtype,
        ...
        try {
            ...
            buf.append("SELECT * FROM transaction WHERE signature IS NOT NULL AND recipient_id = ? AND sender_id <> ? ");
            if (blockTimestamp > 0) {
            ...
            buf.append("UNION ALL SELECT * FROM transaction WHERE signature IS NOT NULL AND sender_id = ? ");
            if (blockTimestamp > 0) {
                buf.append("AND block_timestamp >= ? ");
            }
            if (type >= 0) {
    ...
    public DbIterator<TransactionImpl> getTransactions(Connection con, PreparedStatement pstmt) {
        return new DbIterator<>(con, pstmt, new DbIterator.ResultSetReader<TransactionImpl>() {
            @Override
            public TransactionImpl get(Connection con, ResultSet rs) throws RiseException.ValidationException {
