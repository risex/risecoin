...
import rise.db.VersionedEntityDbTable;
...
public final class Account {
    ...
    static {
        ...
        Rise.getBlockchainProcessor().addListener(new Listener<Block>() {
            @Override
    ...
    private Account(long id) {
        ...
        this.creationHeight = Rise.getBlockchain().getHeight();
    ...
    public static void setRewardRecipientAssignment(Long id, Long recipient) {
    	int currentHeight = Rise.getBlockchain().getLastBlock().getHeight();
    	RewardRecipientAssignment assignment = getRewardRecipientAssignment(id);
    	if(assignment == null) {
    		assignment = new RewardRecipientAssignment(id, id, recipient, (int) (currentHeight + Constants.REWARD_RECIPIENT_ASSIGNMENT_WAIT_TIME));
    	}
    	else {
    		assignment.setRecipient(recipient, (int) (currentHeight + Constants.REWARD_RECIPIENT_ASSIGNMENT_WAIT_TIME));
    	}
    ...
    public static DbIterator<RewardRecipientAssignment> getAccountsWithRewardRecipient(Long recipientId) {
    	return rewardRecipientAssignmentTable.getManyBy(getAccountsWithRewardRecipientClause(recipientId, Rise.getBlockchain().getHeight() + 1), 0, -1);
    }
    ...
    public long getEffectiveBalanceRISE() {
    	return getEffectiveBalanceRISE(getBalanceNQT());
    }

    public long getEffectiveBalanceRISE(long actualBalance) {
        ...
        if (lastBlock.getHeight() < Constants.TRANSPARENT_FORGING_BLOCK_3
            ...
            if (this.creationHeight == 0) {
                return getBalanceNQT() / Constants.ONE_RISE;
            }
            ...
            return (getBalanceNQT() - receivedInlastBlock) / Constants.ONE_RISE;
        }
        if (lastBlock.getHeight() < currentLeasingHeightFrom) {
            return (getGuaranteedBalanceNQT(1440) + getLessorsGuaranteedBalanceNQT()) / Constants.ONE_RISE;
        }
        return getLessorsGuaranteedBalanceNQT() / Constants.ONE_RISE;*/
    	synchronized(BlockchainImpl.getInstance()) {
    		BigInteger effectiveBalance = BigInteger.ZERO;
    		long balance = actualBalance;
    		try(Connection con = Db.db.getConnection();
        			PreparedStatement pstmt = con.prepareStatement("SELECT amount, sender_id, height "
        					+ "FROM transaction "
        					+ "WHERE recipient_id = ? AND height > ? AND height <= ? "
        					+ "ORDER BY height DESC, id")) {
        		int i = 0;
        		pstmt.setLong(++i, getId());
        		pstmt.setInt(++i, Rise.getBlockchain().getHeight() - 20160);
        		pstmt.setInt(++i, Rise.getBlockchain().getHeight());
        		ResultSet rs = pstmt.executeQuery();
        		while(rs.next() && balance > 0) {
        			long currentAmount = Math.min(rs.getLong("amount"), balance);
        			balance -= currentAmount;
        			int currentHeight = Rise.getBlockchain().getHeight() - rs.getInt("height");
        			if(Rise.getBlockchain().getHeight() > 60 && currentHeight < 60 && rs.getLong("sender_id") != getId())
        				continue;
        			int multiplier = 1440 + currentHeight;
        			effectiveBalance = effectiveBalance.add(BigInteger.valueOf(currentAmount).multiply(BigInteger.valueOf(multiplier)).divide(BigInteger.valueOf(1440)));
        		}
        		if(balance > 0) {
        			effectiveBalance = effectiveBalance.add(BigInteger.valueOf(balance * 15));
        		}
        	} catch (SQLException e) {
    			// TODO Auto-generated catch block
    			e.printStackTrace();
    		}
    		return effectiveBalance.longValue() / Constants.ONE_RISE;
    	}
    }
    
    /*public long getActualBalanceForEffective(long effectiveBalance) {
    	long remainingEffective = effectiveBalance;
    	long balance = 0;
    	synchronized(BlockchainImpl.getInstance()) {
    		try(Connection con = Db.db.getConnection();
        			PreparedStatement pstmt = con.prepareStatement("SELECT amount, height "
        					+ "FROM transaction "
        					+ "WHERE recipient_id = ? AND height > ? "
        					+ "ORDER BY height DESC, id")) {
    			int i = 0;
        		pstmt.setLong(++i, getId());
        		pstmt.setInt(++i, Rise.getBlockchain().getHeight() - 40320);
        		ResultSet rs = pstmt.executeQuery();
        		while(rs.next() && remainingEffective > 0) {
        			int currentHeight = Rise.getBlockchain().getHeight() - rs.getInt("height");
        			int multiplier = 720 + currentHeight;
        			long remainingAmountAtHeight = BigInteger.valueOf(remainingEffective).multiply(BigInteger.valueOf(720)).divide(BigInteger.valueOf(multiplier)).longValue();
        			long amountAtHeight = rs.getLong("amount");
        			long amountConsume = Math.min(amountAtHeight, remainingAmountAtHeight);
        			long currentEffective = BigInteger.valueOf(amountConsume).multiply(BigInteger.valueOf(multiplier)).divide(BigInteger.valueOf(720)).longValue();
        			balance += amountConsume;
        			remainingEffective -= currentEffective;
        			if(amountConsume < amountAtHeight)
    ...
    public void addSelfTx(long amount, long blockId, int timestamp) {
    	TransactionImpl.BuilderImpl builder = new TransactionImpl.BuilderImpl((byte)1, getPublicKey(), amount, 0, timestamp, (short)1440, Attachment.ORDINARY_PAYMENT);
    	builder.senderId(getId())
    		.recipientId(getId())
    		.blockId(blockId)
    		...
    		.ecBlockHeight(0)
    		.ecBlockId(0);
    	TransactionImpl transaction = new TransactionImpl((BuilderImpl) builder, true);
    	...
    	try (Connection con = Db.db.getConnection()) {
			...
			e.printStackTrace();
			throw new RuntimeException("Failed to save self-tx in block");
		}
    	
    }
    
    public void addTx(long amount, long blockId, int timestamp, Account sender, Attachment.AbstractAttachment attachment) {
    	TransactionImpl.BuilderImpl builder = new TransactionImpl.BuilderImpl((byte)1, sender.getPublicKey(), amount, 0, timestamp, (short)1440, attachment);
    ...
    public DbIterator<Account> getLessors() {
        return accountTable.getManyBy(getLessorsClause(Rise.getBlockchain().getHeight()), 0, -1);
    }
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations) {
        return getGuaranteedBalanceNQT(numberOfConfirmations, Rise.getBlockchain().getHeight());
    }
    ...
    void leaseEffectiveBalance(long lesseeId, short period) {
        ...
        if (lessee != null && lessee.getPublicKey() != null) {
            int height = Rise.getBlockchain().getHeight();
    ...
    void addToBalanceNQT(long amountNQT) {
        ...
        if(Rise.getBlockchain().getHeight() != 0)
    ...
    private void addToGuaranteedBalanceNQT(long amountNQT) {
        ...
        int blockchainHeight = Rise.getBlockchain().getHeight();
        try (Connection con = Db.db.getConnection();
