...
final class TransactionImpl implements Transaction {
    ...
    static public final class BuilderImpl implements Builder {
        ...
        @Override
        public TransactionImpl build() throws RiseException.NotValidException {
    ...
    private final boolean unsignedtx;
    ...
    private TransactionImpl(BuilderImpl builder) throws RiseException.NotValidException {
        ...
        int effectiveHeight = (height < Integer.MAX_VALUE ? height : Rise.getBlockchain().getHeight());
        long minimumFeeNQT = type.minimumFeeNQT(effectiveHeight, appendagesSize);
        if (builder.feeNQT > 0 && builder.feeNQT < minimumFeeNQT) {
            throw new RiseException.NotValidException(String.format("Requested fee %d less than the minimum fee %d",
        ...
        if ((timestamp == 0 && Arrays.equals(senderPublicKey, Genesis.CREATOR_PUBLIC_KEY))
                ...
                || type == null) {
            throw new RiseException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
                    + ", deadline: " + deadline + ", fee: " + feeNQT + ", amount: " + amountNQT);
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new RiseException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountNQT() != 0) {
                throw new RiseException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient() && version > 0) {
            if (recipientId == 0) {
                throw new RiseException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }

        for (Appendix.AbstractAppendix appendage : appendages) {
            if (! appendage.verifyVersion(this.version)) {
                throw new RiseException.NotValidException("Invalid attachment version " + appendage.getVersion()
                        + " for transaction version " + this.version);
            }
        }
        this.unsignedtx = false;
    }
    
    public TransactionImpl(BuilderImpl builder, boolean unsigned) {
    	this.timestamp = builder.timestamp;
        this.deadline = builder.deadline;
        this.senderPublicKey = builder.senderPublicKey;
        this.recipientId = builder.recipientId;
        ...
        int appendagesSize = this.attachment.getSize();
        this.appendagesSize = appendagesSize;
        this.feeNQT = 0;
        this.unsignedtx = true;
    }
    ...
    public Block getBlock() {
        if (block == null && blockId != 0) {
            block = Rise.getBlockchain().getBlock(blockId);
        }
        return block;
    }
    ...
    public byte[] getBytes() {
        try {
            ...
            if (useNQT()) {
                ...
                buffer.putInt((int) (amountNQT / Constants.ONE_RISE));
                buffer.putInt((int) (feeNQT / Constants.ONE_RISE));
                if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl parseTransaction(byte[] bytes) throws RiseException.ValidationException {
        try {
        ...
        } catch (RiseException.NotValidException|RuntimeException e) {
    ...
    */
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws RiseException.NotValidException {
        try {
            ...
            if (transactionType == null) {
                throw new RiseException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
    ...
    private boolean useNQT() {
        return this.height > Constants.NQT_BLOCK
                ...
                || Rise.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
    }
    ...
    @Override
    public void validate() throws RiseException.ValidationException {
        for (Appendix.AbstractAppendix appendage : appendages) {
            appendage.validate(this);
        }
        long minimumFeeNQT = type.minimumFeeNQT(Rise.getBlockchain().getHeight(), appendagesSize);
        if (feeNQT < minimumFeeNQT) {
            throw new RiseException.NotCurrentlyValidException(String.format("Transaction fee %d less than minimum fee %d at height %d",
                    feeNQT, minimumFeeNQT, Rise.getBlockchain().getHeight()));
        }
        if (Rise.getBlockchain().getHeight() >= Constants.PUBLIC_KEY_ANNOUNCEMENT_BLOCK) {
            ...
            if (recipientId != 0 && recipientId != getSenderId()) {
                ...
                if ((recipientAccount == null || recipientAccount.getPublicKey() == null) && publicKeyAnnouncement == null) {
                    throw new RiseException.NotCurrentlyValidException("Recipient account does not have a public key, must attach a public key announcement");
                }
            }
        }
    }
