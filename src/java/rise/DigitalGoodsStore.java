...
import rise.db.VersionedValuesDbTable;
...
public final class DigitalGoodsStore {
    ...
    private static DbIterator<Purchase> getExpiredPendingPurchases(Block block) {
        ...
        final int previousTimestamp = Rise.getBlockchain().getBlock(block.getPreviousBlockId()).getTimestamp();
    ...
    static void purchase(Transaction transaction,  Attachment.DigitalGoodsPurchase attachment) {
        ...
        if (! goods.isDelisted() && attachment.getQuantity() <= goods.getQuantity() && attachment.getPriceNQT() == goods.getPriceNQT()
                && attachment.getDeliveryDeadlineTimestamp() > Rise.getBlockchain().getLastBlock().getTimestamp()) {
    ...
    static void deliver(Transaction transaction, Attachment.DigitalGoodsDelivery attachment) {
        ...
        seller.addTx(Convert.safeSubtract(totalWithoutDiscount, attachment.getDiscountNQT()), transaction.getBlockId(), transaction.getTimestamp(), buyer, new Attachment.BurstMiningTransactionCoinAgeReset(transaction.getId()));
