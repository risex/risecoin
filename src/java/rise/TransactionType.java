...
public abstract class TransactionType {
    ...
    private static final byte SUBTYPE_BURST_MINING_TRANSACTION_COIN_AGE_RESET = 1;

    private static final byte SUBTYPE_ACCOUNT_CONTROL_EFFECTIVE_BALANCE_LEASING = 0;

    private static final int BASELINE_FEE_HEIGHT = 1; // At release time must be less than current block - 1440
    private static final Fee BASELINE_FEE = new Fee(Constants.ONE_RISE / 10, 0);
    private static final Fee BASELINE_ASSET_ISSUANCE_FEE = new Fee(1000 * Constants.ONE_RISE, 0);
    private static final int NEXT_FEE_HEIGHT = Integer.MAX_VALUE;
    private static final Fee NEXT_FEE = new Fee(Constants.ONE_RISE, 0);
    private static final Fee NEXT_ASSET_ISSUANCE_FEE = new Fee(1000 * Constants.ONE_RISE, 0);
    ...
    public static TransactionType findTransactionType(byte type, byte subtype) {
        switch (type) {
            ...
            case TYPE_BURST_MINING:
            	...
            	case SUBTYPE_BURST_MINING_TRANSACTION_COIN_AGE_RESET:
            		return BurstMining.TRANSACTION_COIN_AGE_RESET;
            	default:
            		return null;
            	}
            default:
    ...
    abstract Attachment.AbstractAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException;

    abstract Attachment.AbstractAttachment parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException;

    abstract void validateAttachment(Transaction transaction) throws RiseException.ValidationException;
    ...
    */
    ...
    public static abstract class Payment extends TransactionType {
        ...
        public static final TransactionType ORDINARY = new Payment() {
            ...
            @Override
            Attachment.EmptyAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return Attachment.ORDINARY_PAYMENT;
            }

            @Override
            Attachment.EmptyAttachment parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
                return Attachment.ORDINARY_PAYMENT;
            }

            @Override
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                if (transaction.getAmountNQT() <= 0 || transaction.getAmountNQT() >= Constants.MAX_BALANCE_NQT) {
                    throw new RiseException.NotValidException("Invalid ordinary payment");
                }
            }
    ...
    public static abstract class Messaging extends TransactionType {
        ...
        public final static TransactionType ARBITRARY_MESSAGE = new Messaging() {
            ...
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (transaction.getAmountNQT() != 0) {
                    throw new RiseException.NotValidException("Invalid arbitrary message: " + attachment.getJSONObject());
                }
            }
        ...
        public static final TransactionType ALIAS_ASSIGNMENT = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasAssignment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.MessagingAliasAssignment(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasAssignment parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (attachment.getAliasName().length() == 0
                    ...
                    throw new RiseException.NotValidException("Invalid alias assignment: " + attachment.getJSONObject());
                }
                String normalizedAlias = attachment.getAliasName().toLowerCase();
                for (int i = 0; i < normalizedAlias.length(); i++) {
                    if (Constants.ALPHABET.indexOf(normalizedAlias.charAt(i)) < 0) {
                        throw new RiseException.NotValidException("Invalid alias name: " + normalizedAlias);
                    }
                }
                Alias alias = Alias.getAlias(normalizedAlias);
                if (alias != null && alias.getAccountId() != transaction.getSenderId()) {
                    throw new RiseException.NotCurrentlyValidException("Alias already owned by another account: " + normalizedAlias);
                }
            }
        ...
        public static final TransactionType ALIAS_SELL = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasSell parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.MessagingAliasSell(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasSell parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                if (transaction.getAmountNQT() != 0) {
                    throw new RiseException.NotValidException("Invalid sell alias transaction: " +
                ...
                if (aliasName == null || aliasName.length() == 0) {
                    throw new RiseException.NotValidException("Missing alias name");
                }
                long priceNQT = attachment.getPriceNQT();
                if (priceNQT < 0 || priceNQT > Constants.MAX_BALANCE_NQT) {
                    throw new RiseException.NotValidException("Invalid alias sell price: " + priceNQT);
                }
                if (priceNQT == 0) {
                    if (Genesis.CREATOR_ID == transaction.getRecipientId()) {
                        throw new RiseException.NotValidException("Transferring aliases to Genesis account not allowed");
                    } else if (transaction.getRecipientId() == 0) {
                        throw new RiseException.NotValidException("Missing alias transfer recipient");
                    }
                }
                final Alias alias = Alias.getAlias(aliasName);
                if (alias == null) {
                    throw new RiseException.NotCurrentlyValidException("Alias hasn't been registered yet: " + aliasName);
                } else if (alias.getAccountId() != transaction.getSenderId()) {
                    throw new RiseException.NotCurrentlyValidException("Alias doesn't belong to sender: " + aliasName);
                }
            }
        ...
        public static final TransactionType ALIAS_BUY = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasBuy parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.MessagingAliasBuy(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasBuy parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (alias == null) {
                    ...
                    throw new RiseException.NotCurrentlyValidException("Alias is owned by account other than recipient: "
                            + Convert.toUnsignedLong(alias.getAccountId()));
                }
                Alias.Offer offer = Alias.getOffer(alias);
                if (offer == null) {
                    throw new RiseException.NotCurrentlyValidException("Alias is not for sale: " + aliasName);
                }
                if (transaction.getAmountNQT() < offer.getPriceNQT()) {
                    ...
                    throw new RiseException.NotCurrentlyValidException(msg);
                }
                if (offer.getBuyerId() != 0 && offer.getBuyerId() != transaction.getSenderId()) {
                    throw new RiseException.NotCurrentlyValidException("Wrong buyer for " + aliasName + ": "
        ...
        public final static TransactionType POLL_CREATION = new Messaging() {
            ...
            @Override
            Attachment.MessagingPollCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.MessagingPollCreation(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingPollCreation parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            @Override
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                if (Rise.getBlockchain().getLastBlock().getHeight() < Constants.VOTING_SYSTEM_BLOCK) {
                    throw new RiseException.NotYetEnabledException("Voting System not yet enabled at height " + Rise.getBlockchain().getLastBlock().getHeight());
                }
                Attachment.MessagingPollCreation attachment = (Attachment.MessagingPollCreation) transaction.getAttachment();
                for (int i = 0; i < attachment.getPollOptions().length; i++) {
                    if (attachment.getPollOptions()[i].length() > Constants.MAX_POLL_OPTION_LENGTH) {
                        throw new RiseException.NotValidException("Invalid poll options length: " + attachment.getJSONObject());
                    }
                }
                if (attachment.getPollName().length() > Constants.MAX_POLL_NAME_LENGTH
                    ...
                    throw new RiseException.NotValidException("Invalid poll attachment: " + attachment.getJSONObject());
                }
            }
        ...
        public final static TransactionType VOTE_CASTING = new Messaging() {
            ...
            @Override
            Attachment.MessagingVoteCasting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.MessagingVoteCasting(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingVoteCasting parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (attachment.getPollId() == 0 || attachment.getPollVote() == null
                    ...
                    throw new RiseException.NotValidException("Invalid vote casting attachment: " + attachment.getJSONObject());
                }
                if (Poll.getPoll(attachment.getPollId()) == null) {
                    throw new RiseException.NotCurrentlyValidException("Invalid poll: " + Convert.toUnsignedLong(attachment.getPollId()));
                }
            }
        ...
        public static final TransactionType HUB_ANNOUNCEMENT = new Messaging() {
            ...
            @Override
            Attachment.MessagingHubAnnouncement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.MessagingHubAnnouncement(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingHubAnnouncement parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            @Override
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                if (Rise.getBlockchain().getLastBlock().getHeight() < Constants.TRANSPARENT_FORGING_BLOCK_7) {
                    throw new RiseException.NotYetEnabledException("Hub terminal announcement not yet enabled at height " + Rise.getBlockchain().getLastBlock().getHeight());
                }
                Attachment.MessagingHubAnnouncement attachment = (Attachment.MessagingHubAnnouncement) transaction.getAttachment();
                if (attachment.getMinFeePerByteNQT() < 0 || attachment.getMinFeePerByteNQT() > Constants.MAX_BALANCE_NQT
                    ...
                    throw new RiseException.NotValidException("Invalid hub terminal announcement: " + attachment.getJSONObject());
                }
                for (String uri : attachment.getUris()) {
                    if (uri.length() > Constants.MAX_HUB_ANNOUNCEMENT_URI_LENGTH) {
                        throw new RiseException.NotValidException("Invalid URI length: " + uri.length());
                    }
        ...
        public static final Messaging ACCOUNT_INFO = new Messaging() {
            ...
            @Override
            Attachment.MessagingAccountInfo parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.MessagingAccountInfo(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAccountInfo parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (attachment.getName().length() > Constants.MAX_ACCOUNT_NAME_LENGTH
                    ...
                    throw new RiseException.NotValidException("Invalid account info issuance: " + attachment.getJSONObject());
                }
            }
    ...
    public static abstract class ColoredCoins extends TransactionType {
        ...
        public static final TransactionType ASSET_ISSUANCE = new ColoredCoins() {
            ...
            @Override
            Attachment.ColoredCoinsAssetIssuance parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.ColoredCoinsAssetIssuance(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetIssuance parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (attachment.getName().length() < Constants.MIN_ASSET_NAME_LENGTH
                    ...
                    throw new RiseException.NotValidException("Invalid asset issuance: " + attachment.getJSONObject());
                }
                String normalizedName = attachment.getName().toLowerCase();
                for (int i = 0; i < normalizedName.length(); i++) {
                    if (Constants.ALPHABET.indexOf(normalizedName.charAt(i)) < 0) {
                        throw new RiseException.NotValidException("Invalid asset name: " + normalizedName);
                    }
                }
            }
        ...
        public static final TransactionType ASSET_TRANSFER = new ColoredCoins() {
            ...
            @Override
            Attachment.ColoredCoinsAssetTransfer parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.ColoredCoinsAssetTransfer(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetTransfer parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (transaction.getAmountNQT() != 0
                    ...
                    throw new RiseException.NotValidException("Invalid asset transfer amount or comment: " + attachment.getJSONObject());
                }
                if (transaction.getVersion() > 0 && attachment.getComment() != null) {
                    throw new RiseException.NotValidException("Asset transfer comments no longer allowed, use message " +
                ...
                if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getQuantityQNT())) {
                    throw new RiseException.NotValidException("Invalid asset transfer asset or quantity: " + attachment.getJSONObject());
                }
                if (asset == null) {
                    throw new RiseException.NotCurrentlyValidException("Asset " + Convert.toUnsignedLong(attachment.getAssetId()) +
                            " does not exist yet");
                }
            }
        ...
        abstract static class ColoredCoinsOrderPlacement extends ColoredCoins {
            ...
            @Override
            final void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                Attachment.ColoredCoinsOrderPlacement attachment = (Attachment.ColoredCoinsOrderPlacement)transaction.getAttachment();
                if (attachment.getPriceNQT() <= 0 || attachment.getPriceNQT() > Constants.MAX_BALANCE_NQT
                        || attachment.getAssetId() == 0) {
                    throw new RiseException.NotValidException("Invalid asset order placement: " + attachment.getJSONObject());
                }
                Asset asset = Asset.getAsset(attachment.getAssetId());
                if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getQuantityQNT())) {
                    throw new RiseException.NotValidException("Invalid asset order placement asset or quantity: " + attachment.getJSONObject());
                }
                if (asset == null) {
                    throw new RiseException.NotCurrentlyValidException("Asset " + Convert.toUnsignedLong(attachment.getAssetId()) +
                            " does not exist yet");
        ...
        public static final TransactionType ASK_ORDER_PLACEMENT = new ColoredCoins.ColoredCoinsOrderPlacement() {
            ...
            @Override
            Attachment.ColoredCoinsAskOrderPlacement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.ColoredCoinsAskOrderPlacement(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAskOrderPlacement parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
        ...
        public final static TransactionType BID_ORDER_PLACEMENT = new ColoredCoins.ColoredCoinsOrderPlacement() {
            ...
            @Override
            Attachment.ColoredCoinsBidOrderPlacement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.ColoredCoinsBidOrderPlacement(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsBidOrderPlacement parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
        ...
        abstract static class ColoredCoinsOrderCancellation extends ColoredCoins {
            ...
            final void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (attachment.getOrderId() == 0) {
                    throw new RiseException.NotValidException("Invalid order cancellation attachment: " + attachment.getJSONObject());
                }
                doValidateAttachment(transaction);
            }

            abstract void doValidateAttachment(Transaction transaction) throws RiseException.ValidationException;
        ...
        public static final TransactionType ASK_ORDER_CANCELLATION = new ColoredCoins.ColoredCoinsOrderCancellation() {
            ...
            @Override
            Attachment.ColoredCoinsAskOrderCancellation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.ColoredCoinsAskOrderCancellation(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAskOrderCancellation parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            @Override
            void doValidateAttachment(Transaction transaction) throws RiseException.ValidationException {
                Attachment.ColoredCoinsAskOrderCancellation attachment = (Attachment.ColoredCoinsAskOrderCancellation) transaction.getAttachment();
                if (Order.Ask.getAskOrder(attachment.getOrderId()) == null) {
                    throw new RiseException.NotCurrentlyValidException("Invalid ask order: " + Convert.toUnsignedLong(attachment.getOrderId()));
                }
            }
        ...
        public static final TransactionType BID_ORDER_CANCELLATION = new ColoredCoins.ColoredCoinsOrderCancellation() {
            ...
            @Override
            Attachment.ColoredCoinsBidOrderCancellation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.ColoredCoinsBidOrderCancellation(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsBidOrderCancellation parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (Order.Bid.getBidOrder(attachment.getOrderId()) == null) {
                    throw new RiseException.NotCurrentlyValidException("Invalid bid order: " + Convert.toUnsignedLong(attachment.getOrderId()));
                }
            }
    ...
    public static abstract class DigitalGoods extends TransactionType {
        ...
        final void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
            if (transaction.getAmountNQT() != 0) {
                throw new RiseException.NotValidException("Invalid digital goods transaction");
            }
            doValidateAttachment(transaction);
        }
        ...
        public static final TransactionType LISTING = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsListing parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.DigitalGoodsListing(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsListing parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (attachment.getName().length() == 0
                    ...
                    throw new RiseException.NotValidException("Invalid digital goods listing: " + attachment.getJSONObject());
                }
            }
        ...
        public static final TransactionType DELISTING = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsDelisting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.DigitalGoodsDelisting(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsDelisting parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (goods != null && transaction.getSenderId() != goods.getSellerId()) {
                    throw new RiseException.NotValidException("Invalid digital goods delisting - seller is different: " + attachment.getJSONObject());
                }
                if (goods == null || goods.isDelisted()) {
                    throw new RiseException.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
        ...
        public static final TransactionType PRICE_CHANGE = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsPriceChange parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.DigitalGoodsPriceChange(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsPriceChange parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (attachment.getPriceNQT() <= 0 || attachment.getPriceNQT() > Constants.MAX_BALANCE_NQT
                    ...
                    throw new RiseException.NotValidException("Invalid digital goods price change: " + attachment.getJSONObject());
                }
                if (goods == null || goods.isDelisted()) {
                    throw new RiseException.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
                            "not yet listed or already delisted");
        ...
        public static final TransactionType QUANTITY_CHANGE = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsQuantityChange parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.DigitalGoodsQuantityChange(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsQuantityChange parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (attachment.getDeltaQuantity() < -Constants.MAX_DGS_LISTING_QUANTITY
                        ...
                        || (goods != null && transaction.getSenderId() != goods.getSellerId())) {
                    throw new RiseException.NotValidException("Invalid digital goods quantity change: " + attachment.getJSONObject());
                }
                if (goods == null || goods.isDelisted()) {
                    throw new RiseException.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
                            "not yet listed or already delisted");
        ...
        public static final TransactionType PURCHASE = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsPurchase parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.DigitalGoodsPurchase(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsPurchase parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (attachment.getQuantity() <= 0 || attachment.getQuantity() > Constants.MAX_DGS_LISTING_QUANTITY
                    ...
                    throw new RiseException.NotValidException("Invalid digital goods purchase: " + attachment.getJSONObject());
                }
                if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
                    throw new RiseException.NotValidException("Only text encrypted messages allowed");
                }
                if (goods == null || goods.isDelisted()) {
                    throw new RiseException.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
                            "not yet listed or already delisted");
                ...
                if (attachment.getQuantity() > goods.getQuantity() || attachment.getPriceNQT() != goods.getPriceNQT()) {
                    throw new RiseException.NotCurrentlyValidException("Goods price or quantity changed: " + attachment.getJSONObject());
                }
                if (attachment.getDeliveryDeadlineTimestamp() <= Rise.getBlockchain().getLastBlock().getTimestamp()) {
                    throw new RiseException.NotCurrentlyValidException("Delivery deadline has already expired: " + attachment.getDeliveryDeadlineTimestamp());
                }
            }
        ...
        public static final TransactionType DELIVERY = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsDelivery parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.DigitalGoodsDelivery(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsDelivery parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (attachment.getGoods().getData().length > Constants.MAX_DGS_GOODS_LENGTH
                    ...
                    throw new RiseException.NotValidException("Invalid digital goods delivery: " + attachment.getJSONObject());
                }
                if (purchase == null || purchase.getEncryptedGoods() != null) {
                    throw new RiseException.NotCurrentlyValidException("Purchase does not exist yet, or already delivered: "
        ...
        public static final TransactionType FEEDBACK = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsFeedback parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.DigitalGoodsFeedback(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsFeedback parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (purchase != null &&
                    ...
                    throw new RiseException.NotValidException("Invalid digital goods feedback: " + attachment.getJSONObject());
                }
                if (transaction.getEncryptedMessage() == null && transaction.getMessage() == null) {
                    throw new RiseException.NotValidException("Missing feedback message");
                }
                if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
                    throw new RiseException.NotValidException("Only text encrypted messages allowed");
                }
                if (transaction.getMessage() != null && ! transaction.getMessage().isText()) {
                    throw new RiseException.NotValidException("Only text public messages allowed");
                }
                if (purchase == null || purchase.getEncryptedGoods() == null) {
                    throw new RiseException.NotCurrentlyValidException("Purchase does not exist yet or not yet delivered");
                }
            }
        ...
        public static final TransactionType REFUND = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsRefund parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.DigitalGoodsRefund(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsRefund parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (attachment.getRefundNQT() < 0 || attachment.getRefundNQT() > Constants.MAX_BALANCE_NQT
                        ...
                        (purchase.getBuyerId() != transaction.getRecipientId()
                                || transaction.getSenderId() != purchase.getSellerId()))) {
                    throw new RiseException.NotValidException("Invalid digital goods refund: " + attachment.getJSONObject());
                }
                if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
                    throw new RiseException.NotValidException("Only text encrypted messages allowed");
                }
                if (purchase == null || purchase.getEncryptedGoods() == null || purchase.getRefundNQT() != 0) {
                    throw new RiseException.NotCurrentlyValidException("Purchase does not exist or is not delivered or is already refunded");
                }
            }
    ...
    public static abstract class AccountControl extends TransactionType {
        ...
        public static final TransactionType EFFECTIVE_BALANCE_LEASING = new AccountControl() {
            ...
            @Override
            Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.AccountControlEffectiveBalanceLeasing(buffer, transactionVersion);
            }

            @Override
            Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
                ...
                if (transaction.getSenderId() == transaction.getRecipientId()
                    ...
                    throw new RiseException.NotValidException("Invalid effective balance leasing: "
                ...
                if (recipientAccount == null
                    ...
                    throw new RiseException.NotCurrentlyValidException("Invalid effective balance leasing: "
    ...
    public static abstract class BurstMining extends TransactionType {
    	...
    	public static final TransactionType REWARD_RECIPIENT_ASSIGNMENT = new BurstMining() {
    		...
    		@Override
            Attachment.BurstMiningRewardRecipientAssignment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.BurstMiningRewardRecipientAssignment(buffer, transactionVersion);
            }

            @Override
            Attachment.BurstMiningRewardRecipientAssignment parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
    		...
    		@Override
    		void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
    			long height = Rise.getBlockchain().getLastBlock().getHeight() + 1;
    			...
    			if(rewardAssignment != null && rewardAssignment.getFromHeight() >= height) {
    				throw new RiseException.NotValidException("Cannot reassign reward recipient before previous goes into effect: " + transaction.getJSONObject());
    			}
    			Account recip = Account.getAccount(transaction.getRecipientId());
    			if(recip == null || recip.getPublicKey() == null) {
    				throw new RiseException.NotValidException("Reward recipient must have public key saved in blockchain: " + transaction.getJSONObject());
    			}
    			if(transaction.getAmountNQT() != 0 || transaction.getFeeNQT() != Constants.ONE_RISE) {
    				throw new RiseException.NotValidException("Reward recipient assisnment transaction must have 0 send amount and 1 fee: " + transaction.getJSONObject());
    			}
    		}
    	...
    	public static final TransactionType TRANSACTION_COIN_AGE_RESET = new BurstMining() {
    		
    		@Override
    		public final byte getSubtype() {
    			return TransactionType.SUBTYPE_BURST_MINING_TRANSACTION_COIN_AGE_RESET;
    		}
    		
    		@Override
            Attachment.BurstMiningTransactionCoinAgeReset parseAttachment(ByteBuffer buffer, byte transactionVersion) throws RiseException.NotValidException {
                return new Attachment.BurstMiningTransactionCoinAgeReset(buffer, transactionVersion);
            }

            @Override
            Attachment.BurstMiningTransactionCoinAgeReset parseAttachment(JSONObject attachmentData) throws RiseException.NotValidException {
                return new Attachment.BurstMiningTransactionCoinAgeReset(attachmentData);
            }
    		...
    		@Override
    		void validateAttachment(Transaction transaction) throws RiseException.ValidationException {
    			throw new RiseException.NotValidException("never validates");
    		}
