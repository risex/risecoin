...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public final void checkAvailable(int height) {
        if (multiversion && height < Rise.getBlockchainProcessor().getMinRollbackHeight()) {
            throw new IllegalArgumentException("Historical data as of height " + height +" not available, set rise.trimDerivedTables=false and re-scan");
        }
    }
